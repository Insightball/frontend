...
export default function MatchBoard(){
  ...
  // Player down — captures pointer so move/up always fire even outside element
  ...
  const hs=mob?22:16
  const tbS=a=>({padding:mob?'5px 8px':'6px 12px',background:a?'#c9a227':'transparent',border:a?'none':'1px solid rgba(255,255,255,0.10)',color:a?'#0a0908':'rgba(245,242,235,0.55)',fontFamily:F,fontSize:mob?8:9,letterSpacing:'.08em',textTransform:'uppercase',cursor:'pointer',fontWeight:a?700:400})
  const fS=a=>({padding:'4px 9px',background:a?'rgba(201,162,39,0.12)':'transparent',border:'1px solid '+(a?'rgba(201,162,39,0.35)':'rgba(255,255,255,0.07)'),color:a?'#c9a227':'rgba(245,242,235,0.40)',fontFamily:F,fontSize:9,cursor:'pointer'})
  const bar={display:'flex',flexWrap:'wrap',gap:4,padding:'8px 14px',background:'#0a0908',borderTop:'1px solid rgba(255,255,255,0.05)',alignItems:'center'}
  const all=shA?[...pl,...aw]:pl
  const a=act.current

  return(<div style={{fontFamily:F,background:'#f5f2eb',width:'100%',maxWidth:560,margin:'0 auto'}}>
    <div style={{display:'flex',flexWrap:'wrap',gap:mob?4:6,padding:'10px 14px',background:'#0a0908',alignItems:'center'}}>
    ...
    {shZP&&(<div style={{padding:'12px 14px',background:'#0d0c0a',borderTop:'1px solid rgba(255,255,255,0.05)'}}>
      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
        <div style={{position:'relative'}}>
          <button onClick={e=>{e.stopPropagation();setShZCP(!shZCP)}} style={{width:20,height:20,borderRadius:4,background:azc.stroke.replace('0.55','0.85'),border:'2px solid rgba(255,255,255,0.15)',cursor:'pointer'}}/>
        ...
        <span style={{fontFamily:F,fontSize:8,color:'rgba(245,242,235,0.40)',letterSpacing:'.08em',textTransform:'uppercase'}}>Dessinez ou choisissez :</span>
      </div>
      <div style={{display:'grid',gridTemplateColumns:mob?'1fr 1fr':'1fr 1fr 1fr',gap:4}}>
        {Object.entries(PRESETS).map(([k,p])=>(<button key={k} onClick={()=>addPreset(k)} style={{padding:'6px 8px',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',cursor:'pointer',textAlign:'left'}} onMouseEnter={e=>{e.currentTarget.style.borderColor='rgba(201,162,39,0.3)';e.currentTarget.style.background='rgba(201,162,39,0.05)'}} onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(255,255,255,0.07)';e.currentTarget.style.background='rgba(255,255,255,0.03)'}}><div style={{fontFamily:F,fontSize:8,color:'#c9a227',letterSpacing:'.04em',marginBottom:2}}>{p.label}</div><div style={{fontFamily:F,fontSize:7,color:'rgba(245,242,235,0.25)'}}>{p.desc}</div></button>))}
      </div>
    </div>)}

    <div style={bar}>
      <div style={{position:'relative'}}><button onClick={e=>{e.stopPropagation();setShowHCP(!showHCP);setShowACP(false)}} style={{width:18,height:18,borderRadius:'50%',background:hCol.bg,border:'2px solid rgba(255,255,255,0.2)',cursor:'pointer',marginRight:6}}/>{showHCP&&<CPick cur={hCol} onChange={setHCol} onClose={()=>setShowHCP(false)}/>}</div>
      <span style={{fontSize:8,letterSpacing:'.14em',textTransform:'uppercase',color:'rgba(245,242,235,0.30)',marginRight:4,fontFamily:F}}>Dom</span>
      {Object.keys(FM).map(k=><button key={k} style={fS(fm===k)} onClick={()=>applyF(k,'home')}>{k}</button>)}
    </div>
    {shA&&(<div style={bar}>
      <div style={{position:'relative'}}><button onClick={e=>{e.stopPropagation();setShowACP(!showACP);setShowHCP(false)}} style={{width:18,height:18,borderRadius:'50%',background:aCol.bg,border:'2px solid rgba(255,255,255,0.2)',cursor:'pointer',marginRight:6}}/>{showACP&&<CPick cur={aCol} onChange={setACol} onClose={()=>setShowACP(false)}/>}</div>
      <span style={{fontSize:8,letterSpacing:'.14em',textTransform:'uppercase',color:'rgba(245,242,235,0.30)',marginRight:4,fontFamily:F}}>Ext</span>
      {Object.keys(FM).map(k=><button key={'a'+k} style={fS(afm===k)} onClick={()=>applyF(k,'away')}>{k}</button>)}
    </div>)}

    <div ref={pitchRef} style={{position:'relative',width:'100%',paddingBottom:'154.4%',overflow:'hidden',cursor:tool==='arrow'||tool==='zone'?'crosshair':'default',touchAction:'none',userSelect:'none'}}
      ...
      <div style={{position:'absolute',inset:0}}>
        ...
        {zones.map(z=>{const isSel=sel===z.id;return(<div key={z.id}>
          <div style={{position:'absolute',left:z.x+'%',top:z.y+'%',width:z.w+'%',height:z.h+'%',background:z.fill,border:isSel?'3px solid '+z.stroke.replace('0.55','0.9'):'2px dashed '+z.stroke,borderRadius:2,zIndex:3,pointerEvents:'none',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:isSel?'0 0 0 3px '+z.stroke.replace('0.55','0.2'):'none'}}>
            {eZ!==z.id&&z.label&&<span style={{fontFamily:F,fontSize:mob?7:9,fontWeight:700,color:z.txt||z.stroke,letterSpacing:'.1em',textTransform:'uppercase',textShadow:'0 1px 4px rgba(0,0,0,0.5)',padding:'0 4px',textAlign:'center',lineHeight:1.3,wordBreak:'break-word'}}>{z.label}</span>}
          </div>
          {eZ===z.id&&<div style={{position:'absolute',left:z.x+'%',top:z.y+'%',width:z.w+'%',height:z.h+'%',zIndex:25,display:'flex',alignItems:'center',justifyContent:'center'}}><input autoFocus value={ezV} onChange={e=>setEzV(e.target.value)} onBlur={confirmEZ} onKeyDown={e=>{if(e.key==='Enter')confirmEZ()}} style={{background:'rgba(0,0,0,0.7)',border:'1px solid '+z.stroke,color:'#f5f2eb',fontFamily:F,fontSize:mob?9:10,textAlign:'center',padding:'4px 10px',outline:'none',letterSpacing:'.08em',textTransform:'uppercase',width:'85%',maxWidth:200,borderRadius:3}}/></div>}
          {isSel&&<div style={{position:'absolute',left:z.x+'%',top:z.y+'%',width:z.w+'%',height:z.h+'%',zIndex:20,cursor:'grab'}} onPointerDown={e=>onSelZDown(e,z)}>
            <div style={{position:'absolute',top:-11,right:-11,width:mob?26:22,height:mob?26:22,borderRadius:'50%',background:'#dc2626',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',boxShadow:'0 2px 8px rgba(0,0,0,0.5)',border:'2px solid rgba(255,255,255,0.4)',zIndex:30}} onPointerDown={e=>{e.preventDefault();e.stopPropagation();setZones(p=>p.filter(q=>q.id!==z.id));setSel(null)}}><span style={{color:'#fff',fontSize:mob?14:12,fontWeight:700,lineHeight:1}}>✕</span></div>
            <div style={{position:'absolute',top:-11,left:-11,width:mob?26:22,height:mob?26:22,borderRadius:'50%',background:z.stroke.replace('0.55','0.85'),display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',boxShadow:'0 2px 8px rgba(0,0,0,0.5)',border:'2px solid rgba(255,255,255,0.4)',zIndex:30}} onPointerDown={e=>{e.preventDefault();e.stopPropagation();setEZ(z.id);setEzV(z.label)}}><span style={{color:'#fff',fontSize:mob?12:10,fontWeight:700,lineHeight:1}}>✎</span></div>
          ...
          {!isSel&&tool==='move'&&<div style={{position:'absolute',left:z.x+'%',top:z.y+'%',width:z.w+'%',height:z.h+'%',zIndex:4,cursor:'pointer'}} onDoubleClick={e=>{e.preventDefault();e.stopPropagation();setSel(z.id)}}/>}
        </div>)})}
        ...
        {all.map(p=>{const c=p.tm==='home'?hCol:aCol;const isDr=a.type==='player'&&a.d&&a.d.id===p.id;const isEd=eP===p.id;return(
          <div key={p.id} style={{position:'absolute',left:p.x+'%',top:p.y+'%',transform:'translate(-50%,-50%)',zIndex:isDr?100:10,cursor:tool==='move'?'grab':'default',touchAction:'none',userSelect:'none'}} onPointerDown={e=>onPlDown(e,p)} onDoubleClick={()=>{setEP(p.id);setEV(p.nm)}}>
            <div style={{width:mob?26:30,height:mob?26:30,borderRadius:'50%',background:c.bg,border:'2.5px solid '+((['#ffffff','#eab308','#06b6d4'].indexOf(c.bg)>=0)?'rgba(0,0,0,0.15)':'rgba(255,255,255,0.25)'),display:'flex',alignItems:'center',justifyContent:'center',boxShadow:isDr?'0 4px 16px rgba(0,0,0,0.6)':'0 2px 6px rgba(0,0,0,0.35)',transform:isDr?'scale(1.15)':'scale(1)',transition:isDr?'none':'transform .12s'}}>
              <span style={{fontFamily:F,fontSize:mob?9:10,fontWeight:700,color:c.t,lineHeight:1}}>{p.n}</span>
            </div>
            {isEd?<input autoFocus value={eV} onChange={e=>setEV(e.target.value)} onBlur={confirmEP} onKeyDown={e=>e.key==='Enter'&&confirmEP()} style={{position:'absolute',top:'100%',left:'50%',transform:'translateX(-50%)',marginTop:2,width:56,padding:'2px 4px',background:'#0a0908',border:'1px solid #c9a227',color:'#f5f2eb',fontFamily:F,fontSize:8,textAlign:'center',outline:'none'}}/>
            :p.nm&&<div style={{position:'absolute',top:'100%',left:'50%',transform:'translateX(-50%)',marginTop:1,background:'rgba(0,0,0,0.65)',padding:'1px 5px',whiteSpace:'nowrap',borderRadius:2}}><span style={{fontFamily:F,fontSize:mob?6:7,color:'rgba(255,255,255,0.85)',letterSpacing:'.04em'}}>{p.nm}</span></div>}
          </div>
        )})}
      </div>
    </div>

    <div style={{display:'flex',gap:6,padding:'10px 14px',background:'#0a0908',borderTop:'1px solid rgba(255,255,255,0.05)'}}>
      <button onClick={async()=>{setSel(null);await new Promise(r=>setTimeout(r,50));try{const{default:h2c}=await import('https://cdn.jsdelivr.net/npm/html2canvas@1.4.1/dist/html2canvas.esm.js');const c=await h2c(pitchRef.current,{backgroundColor:'#1a1916',scale:2,useCORS:true});const l=document.createElement('a');l.download='insightball-matchboard-'+Date.now()+'.png';l.href=c.toDataURL('image/png');l.click()}catch(e){alert('Réessayez')}}} style={{padding:'8px 18px',background:'#c9a227',border:'none',color:'#0a0908',fontFamily:F,fontSize:9,letterSpacing:'.1em',textTransform:'uppercase',fontWeight:700,cursor:'pointer'}}>📸 Exporter PNG</button>
      <button onClick={resetAll} style={{padding:'8px 18px',background:'transparent',border:'1px solid rgba(255,255,255,0.10)',color:'rgba(245,242,235,0.5)',fontFamily:F,fontSize:9,letterSpacing:'.1em',textTransform:'uppercase',cursor:'pointer'}}>↺ Reset</button>
    </div>
    <div style={{padding:'8px 14px',background:'#0a0908',borderTop:'1px solid rgba(255,255,255,0.05)'}}><span style={{fontFamily:F,fontSize:7,color:'rgba(245,242,235,0.22)',letterSpacing:'.06em'}}>Double-clic zone → sélectionner · Clic terrain → désélectionner</span></div>
  </div>)
}
