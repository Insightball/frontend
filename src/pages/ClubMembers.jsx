/**
 * ClubMembers.jsx
 * Gestion des membres du club — invitations, rôles, suppressions
 * Accessible uniquement aux admins du club
 */
import { useState, useEffect } from 'react'
import { UserPlus, Trash2, Edit2, X, Check, Mail, ChevronDown } from 'lucide-react'
...
function RoleBadge({ role }) {
  ...
  return (
    <span style={{
      ...
      padding: '3px 10px', background: color + '18', color, border: `1px solid ${color}30`
    }}>
...
function Avatar({ name }) {
  ...
  return (
    <div style={{
      width: 38, height: 38, background: G.goldBg, border: `1px solid ${G.goldBdr}`,
...
function InviteModal({ onClose, onSuccess }) {
    ...
    try {
      const res = await fetch(`${API}/invite`, {
        ...
        body: JSON.stringify({
          email: form.email,
          role: form.role,
          category: form.role === 'coach' ? form.category : null,
        })
      })
  ...
  const inputStyle = {
    width: '100%', background: '#fff', border: `1px solid rgba(15,15,13,0.12)`,
    padding: '11px 14px', color: '#0f0f0d', fontFamily: G.mono, fontSize: 12,
    outline: 'none', boxSizing: 'border-box', transition: 'border-color .15s',
  }
  const labelStyle = {
    fontFamily: G.mono, fontSize: 8, letterSpacing: '.16em', textTransform: 'uppercase',
    color: G.muted, marginBottom: 8, display: 'block',
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: `1px solid rgba(15,15,13,0.09)`, borderTop: `2px solid ${G.gold}`,
      }}>
        {/* Header */}
        <div style={{ padding: '20px 24px', borderBottom: `1px solid rgba(15,15,13,0.09)`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontFamily: G.display, fontSize: 22, textTransform: 'uppercase', color: '#0f0f0d', letterSpacing: '.02em' }}>Inviter</div>
          </div>
        ...
        <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 18 }}>
          {error && (
            <div style={{ padding: '10px 14px', background: 'rgba(239,68,68,0.06)', border: `1px solid rgba(239,68,68,0.2)`, fontFamily: G.mono, fontSize: 11, color: G.red }}>
              {error}
            </div>
          )}
          ...
          <div>
            ...
            <input
              type="email" placeholder="coach@monclub.fr"
              value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))}
              style={inputStyle}
              onFocus={e => e.target.style.borderColor = G.gold}
              onBlur={e => e.target.style.borderColor = 'rgba(15,15,13,0.12)'}
            />
          </div>
          ...
          <div>
            ...
            <select
              value={form.role} onChange={e => setForm(p => ({ ...p, role: e.target.value, category: '' }))}
          ...
          <div style={{ display: 'flex', gap: 10, paddingTop: 4 }}>
            <button onClick={onClose} style={{
              ...
              border: `1px solid rgba(15,15,13,0.12)`, fontFamily: G.mono, fontSize: 9,
              letterSpacing: '.12em', textTransform: 'uppercase', color: G.muted, cursor: 'pointer',
            }}>
              Annuler
            </button>
            <button onClick={handleSubmit} disabled={loading} style={{
              flex: 2, padding: '11px', background: loading ? 'rgba(201,162,39,0.4)' : G.gold,
              border: 'none', fontFamily: G.mono, fontSize: 9, letterSpacing: '.12em',
...
function EditModal({ member, onClose, onSuccess }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ background: '#fff', width: '100%', maxWidth: 380, border: `1px solid rgba(15,15,13,0.09)`, borderTop: `2px solid ${G.gold}` }}>
        <div style={{ padding: '20px 24px', borderBottom: `1px solid rgba(15,15,13,0.09)`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontFamily: G.display, fontSize: 20, textTransform: 'uppercase', color: '#0f0f0d' }}>Modifier</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer' }}><X size={16} color={G.muted} /></button>
        </div>
        <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ fontFamily: G.mono, fontSize: 11, color: '#0f0f0d', marginBottom: 4 }}>{member.user_name || member.email}</div>
          <div>
            <div style={{ fontFamily: G.mono, fontSize: 8, letterSpacing: '.16em', textTransform: 'uppercase', color: G.muted, marginBottom: 8 }}>Rôle</div>
          ...
          {form.role === 'coach' && (
            <div>
              <div style={{ fontFamily: G.mono, fontSize: 8, letterSpacing: '.16em', textTransform: 'uppercase', color: G.muted, marginBottom: 8 }}>Catégorie</div>
          ...
          <div style={{ display: 'flex', gap: 10, paddingTop: 4 }}>
            <button onClick={onClose} style={{ flex: 1, padding: '10px', background: 'transparent', border: `1px solid rgba(15,15,13,0.12)`, fontFamily: G.mono, fontSize: 9, letterSpacing: '.1em', textTransform: 'uppercase', color: G.muted, cursor: 'pointer' }}>Annuler</button>
...
export default function ClubMembers() {
  ...
  const thStyle = {
    fontFamily: G.mono, fontSize: 8, letterSpacing: '.16em', textTransform: 'uppercase',
    color: G.muted, padding: '10px 16px', textAlign: 'left',
    borderBottom: `1px solid rgba(15,15,13,0.09)`, whiteSpace: 'nowrap',
  }
  const tdStyle = {
    fontFamily: G.mono, fontSize: 12, color: '#0f0f0d',
    padding: '14px 16px', borderBottom: `1px solid rgba(15,15,13,0.06)`,
  ...
  return (
    <DashboardLayout>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 1, flexWrap: 'wrap', gap: 12 }}>
        {/* Filtres */}
        <div style={{ display: 'flex', gap: 1, background: 'rgba(15,15,13,0.09)' }}>
          {[
            { key: 'all', label: 'Tous' },
            ...
            <button key={key} onClick={() => setFilter(key)} style={{
              padding: '9px 16px', background: filter === key ? G.gold : '#fff',
        ...
        <button onClick={() => setShowInvite(true)} style={{
          ...
          letterSpacing: '.12em', textTransform: 'uppercase', color: '#0f0f0d',
      ...
      {/* Table */}
      <div style={{ background: '#fff', border: `1px solid rgba(15,15,13,0.09)`, overflowX: 'auto' }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: '48px', fontFamily: G.mono, fontSize: 10, letterSpacing: '.12em', textTransform: 'uppercase', color: G.muted }}>
            Chargement...
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '56px 24px' }}>
            <div style={{ fontFamily: G.display, fontSize: 28, textTransform: 'uppercase', color: 'rgba(15,15,13,0.15)', marginBottom: 12 }}>Aucun membre</div>
            ...
            {filter === 'all' && (
              <button onClick={() => setShowInvite(true)} style={{
                padding: '10px 20px', background: G.gold, border: 'none',
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'rgba(15,15,13,0.02)' }}>
                <th style={thStyle}>Membre</th>
                <th style={thStyle}>Rôle</th>
                <th style={thStyle}>Catégorie</th>
                <th style={thStyle}>Statut</th>
                <th style={thStyle}>Invité le</th>
                ...
                <tr key={member.id}
                  ...
                  <td style={tdStyle}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div>
                        <div style={{ fontFamily: G.mono, fontSize: 12, color: '#0f0f0d', letterSpacing: '.04em' }}>
                          {member.user_name || '—'}
                        </div>
                        <div style={{ fontFamily: G.mono, fontSize: 10, color: G.muted, letterSpacing: '.04em', marginTop: 2 }}>
                          {member.email}
                        </div>
                      </div>
                    </div>
                  ...
                  <td style={tdStyle}>
                    <span style={{ fontFamily: G.mono, fontSize: 11, color: member.category ? '#0f0f0d' : G.muted }}>
                      {member.category || '—'}
                    </span>
                  </td>
                  <td style={tdStyle}><StatusBadge status={member.status} /></td>
                  <td style={tdStyle}>
                    <span style={{ fontFamily: G.mono, fontSize: 10, color: G.muted }}>
                      {new Date(member.invited_at).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                    </span>
                  </td>
                  <td style={{ ...tdStyle, textAlign: 'right' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 8 }}>
                      <button onClick={() => setEditMember(member)} style={{
                        ...
                        border: `1px solid rgba(15,15,13,0.12)`, cursor: 'pointer',
                        display: 'flex', alignItems: 'center', gap: 5,
                        fontFamily: G.mono, fontSize: 9, letterSpacing: '.1em', textTransform: 'uppercase', color: G.muted,
                        transition: 'all .15s',
                      }}
                        onMouseEnter={e => { e.currentTarget.style.borderColor = G.goldBdr; e.currentTarget.style.color = G.gold }}
                        onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(15,15,13,0.12)'; e.currentTarget.style.color = G.muted }}>
