...
// CLUB et CLUB_PRO → ClubSettings
// COACH (et tout autre cas) → CoachSettings
function SettingsRoute() {
  const { user } = useAuth()
  if (!user) return null
  const isClub = user.plan === 'CLUB' || user.plan === 'CLUB_PRO'
  return isClub ? <ClubSettings /> : <CoachSettings />
}
      ...
      <Router>
        <Suspense fallback={<PageLoader />}>
          <Routes>
            ...
            {/* Invitation club — publique, pas de ProtectedRoute */}
            <Route path="/club-invite/:token" element={<ClubInvite />} />

            {/* Invitation membre & récupération */}
